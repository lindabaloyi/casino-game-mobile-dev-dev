...
} from "../../src/utils/strategicCaptureAnalysis";
...
export function useHandCardDragHandler({
  ...
  const handleDragEnd = useCallback(
    (
      ...
      logger.info(
        `Hand card drag end: ${draggedItem.card.rank}${draggedItem.card.suit}`,
      );
      ...
      logger.info(
        `Checking contact at position: (${dropPosition.x.toFixed(1)}, ${dropPosition.y.toFixed(1)})`,
      );
      ...
      if (contact) {
        logger.info(
          `✅ Found contact: ${contact.id} (${contact.type}) at ${contact.distance.toFixed(1)}px`,
        );
        ...
        if (
          ...
          if (strategicOptions) {
            logger.info(
              `🎯 Strategic capture options found: ${strategicOptions.length} options`,
            );
            // Convert strategic options to ActionOption format for the modal
            const modalOptions = strategicOptions.map((option) => ({
              type: option.type as "capture" | "extendBuild",
              label: option.label,
        ...
        if (
          ...
          try {
            const ruleResult = determineActions(
              draggedItemForRules,
              targetInfoForRules,
              gameState,
            );
            ...
            logger.info(
              `📊 Rule engine result: ${ruleResult.actions.length} actions, requiresModal: ${ruleResult.requiresModal}`,
            );

            // For now, if multiple actions exist, we'll need modal handling
            // For single actions, use it directly
            if (
              ruleResult.actions.length === 1 &&
              !ruleResult.requiresModal &&
              ruleResult.dataPackets.length === 0
            ) {
              action = ruleResult.actions[0];
              if (action) {
                logger.info(
                  "✅ Using single action from rule engine:",
                  action.type,
                );
              } else {
                logger.error(
                  "❌ Rule engine returned null action despite length check",
                );
              }
            } else {
              logger.warn(
                "🎯 Multiple actions/options require modal - not implemented yet",
              );
        ...
        logger.info(
          "ℹ️ No contact found - empty table drop, sending trail action",
        );
        ...
        if (!isMyTurn) {
          ...
          setErrorModal({
            visible: true,
            title: "Turn Error",
            message: "Cannot trail - not your turn.",
          });
          return { validContact: false };
        }
