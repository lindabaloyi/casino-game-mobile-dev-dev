 ...
 * Total available points per deal: 11 (standard) or 7 (three-hands)
 * 
 * Scoring rules:
 * - 10 of diamonds (dix): 2 points
 * - 2 of spades: 1 point
 * - Each Ace: 1 point
 * - Spades bonus: 6+ spades = +2 points (NOT for three-hands)
 * - Card count bonus: 21+ cards = +2 points, exactly 20 cards = +1 point (NOT for three-hands)
 */
...
function calculatePlayerScore(capturedCards, gameType = 'standard') {
  ...
  // 1. Individual card points (base points - always included)
...
function getScoreBreakdown(capturedCards, gameType = 'standard') {
  ...
  // Bonuses NOT included for three-hands mode
  if (!isThreeHands) {
    if (spadeCount >= 6) {
      spadeBonus = 2;
    }
